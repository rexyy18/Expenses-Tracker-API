...
let expenses = [ // In-memory array to store expense data
  { id: 1, amount: 200, category: "Education", date: "17-03-2025" },
  { id: 2, amount: 234, category: "House", date: "12-11-2025" },
];

// Home route
app.get("/", (req,res)=>{ // Handle GET requests to root path
  res.status(200).send( // Send HTML response with 200 status
    "<h1>GROUP 3 PROJECT WORK</h1>"
  )
})

app.get("/api/", (req, res) => { // Handle GET requests to /api path
  res.status(200).json({ // Send JSON response with 200 status
...
// Get all expenses
app.get("/api/expenses", (req, res) => { // Handle GET all expenses
  res.status(200).json(expenses); // Return all expenses as JSON
});

// Get a single Expenses
app.get("/api/expenses/:id", (req, res) => { // Handle GET single expense by ID
  const Id = parseInt(req.params.id); // Convert URL parameter to integer
  const expense = expenses.find((t) => t.id === Id); // Find expense by ID
  if (!expense) return res.status(200).json({ message: "Not found" }); // Return if not found
  res.status(200).json(expense); // Return found expense
});

// Add a new expenses
app.post("/api/expenses", (req, res) => { // Handle POST to create new expense
  const { amount, category, date } = req.body; // Extract data from request body
  if (!amount || !category || !date === undefined) // Check if required fields exist
...
// Update expenses
app.patch("/api/expenses/:id", (req, res) => { // Handle PATCH to update expense
  const Id = parseInt(req.params.id); // Convert URL parameter to integer
  const expense = expenses.find((t) => t.id === Id); // Find expense by ID
...
app.delete("/api/expenses/:id", (req, res) => { // Handle DELETE to remove expense
  const Id = parseInt(req.params.id); // Convert URL parameter to integer
  ...
  if (expenses.length === initiallength) // Check if array length changed
